...
class ModalAddCabinetBusiness extends Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  getLocationByBusiness = async () => {
    let response = await getLocationByBusinessService(this.state.businessId);
    this.setState({
    ...
    if (
      ...
      try {
        ...
        if (response && response.errCode === 0) {
          this.props.createCabinetBusiness(this.state);
        } else {
...
export default injectIntl(ModalAddCabinetBusiness);
